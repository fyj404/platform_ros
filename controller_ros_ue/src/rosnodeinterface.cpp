...
// C/C++ source code generated on : Tue Aug 23 08:05:35 2022
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"controller_ros_ue\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(controller_ros_ue_M, (NULL));
        controller_ros_ue_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(controller_ros_ue_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(controller_ros_ue_M)
                == (NULL)) || rtmGetStopRequested(controller_ros_ue_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(controller_ros_ue_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
